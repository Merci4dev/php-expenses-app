...
<link rel="stylesheet" href="<?php echo constant('URL') ?>public/css/history.css">
            ...
            <div id="table-container">
                <table width="100%" cellpadding="0">
                    <thead>
                        ...
                        <th data-sort="category">Categoríes</th>
    ...
    <script>
        ...
        async function getData(){
            data = await fetch('http://localhost:80/expenses/expenses/getHistoryJSON')
            .then(res =>res.json())
            .then(json => json);
            this.copydata = [...this.data];
            console.table(data);
        ...
        function renderData(data){
            ...
            data.forEach(item => { 
                ...
                databody.innerHTML += `<tr>
                        ...
                        <td>${item.date}</td>
                        <td>$${item.amount}</td>
                        <td><a href="http://localhost:80/expenses/expenses/delete/${item.id}">Eliminar</a></td>
                    </tr>`;
            });
        }
